...
| `auditLeadTabHeaders()` | **Read-only.** For every lead tab: data-row count, live header row cell by cell, and a per-column diff against `expectedHeadersFor()`. Flags headers the runtime looks up by name (`Lead ID`, `Category`, `Heard About`) when absent, and says whether a drifted tab is a safe rewrite (empty) or needs realignment (has rows). Run this before trusting any tab's layout. |
