...
|---|---|---|
| `sendDailyDigest` | daily, `atHour(18)` (6 pm CT) | **Branded HTML** digest (as of 2026-07-20; was plain text) of leads whose **Timestamp** falls on today's CT calendar date (ISO parsed, formatted to CT `MM/dd/yyyy`), to `NOTIFY_EMAILS`. Silent if none. Reads the single **`Leads`** table and pulls Asset Class + Booking out of the `Details` blob. Renders `TEMPLATE_DAILY_DIGEST` with `htmlBody` + inline `logo`; one card per lead via `buildDigestLeadCard`. **Each card shows both `Source` (arrival channel column, blank→`Direct`, `QR`) and a separate `Heard about us` row from the `Heard About` column** — the latter omitted when blank (EAO), so the self-reported attribution (LinkedIn, Personal referral) is surfaced without being conflated with origin. **Read-only → no lock.** |
