...
### Stage 8 notes — every lead-DATA function is migrated; what is left is SETUP + TEARDOWN

**YES — with Stage 8, every function that reads or writes lead DATA is migrated.**
Form submission, dedupe, referral matching + stats, the edit trigger, the cold sweep,
the daily digest, and the monthly summary all run against the one `Leads` table under
the switch. The runtime is done.

**What remains is not lead-data logic — it is Stage 9 (setup/teardown) and the
cutover:**

- **`setupSpreadsheet`** — creates 11 legacy tabs; must create `Leads` + `Referrals` +
  `Subscribers`. **This one must be RUN BY HAND from the Apps Script editor** — `clasp
  deploy` does not create tabs, and skipping it is exactly what broke EAO. It is also
  what actually makes the `Leads` tab *exist*, which is why it is the last thing before
  the switch can flip.
- **The `LEAD_TYPES` registry** — `.tab` and `.tabColor` become meaningless and come
  out. `.category`, `.contactGroup`, `.normalizer`, `.seedReportsEnabled`, and Stage
  6's `.detailsFields`/`.detailsFrom` all stay.
- **`resolveCols` / `COLS` / `LEAD_HEADERS`** — the legacy column layer. Once every
  `xxxLegacy` body is deleted at cutover, its only readers are gone. `resolveUnifiedCols`
  / `UCOLS` / `UNIFIED_LEAD_HEADERS` replace it.

**Obsoleted AT CUTOVER (deleted, not before — they are the rollback path):** every
`xxxLegacy` body across Stages 1-8; the entire **header audit/repair family** (existed
only because nine tabs could drift independently — one table, one header); the entire
**EAO backfill family** (existed only because rows fell into the gap between Lifetime
Leads and a missing category tab — no category tabs, no gap); `setCategoryTabStatus`;
and `reportsEnabledIndex` / `headerIndex` (the per-tab-extra resolver, now that
`Reports Enabled` is a standard column).

**Two things confirmed in this stage, not assumed:**

1. **Neither digest takes the script lock, and neither should.** Both are read-only —
   they read rows and send email, and write no cell — so there is no row they could
   race anyone on. A test wires `getScriptLock()` to *throw*, so a future edit that
   introduced a lock (or a sheet write) would fail rather than pass quietly.
2. **`sendDailyDigest` reads Lifetime Leads, not Active Leads.** The plan's §3 said
   Active Leads; the code says Lifetime Leads. The code is right; §3 is corrected. This
   is the kind of drift the standing rule ("verify against the code, never inherit a
   description") exists to catch — and it went uncaught precisely because this function
   had no tests until now.

**One migration detail worth noting for the digest:** Asset Class, Booking Date, and
Booking Time were top-level columns in the legacy schema but are `Details` keys now
(Stage 6). The unified digest parses them back out of the blob — reading them
...
| ✅ **`sendMonthlyReferralSummaries`** — **MIGRATED 2026-07-14 (Stage 8).** Filters the one table on `Category = 'Referral Partner'`; `Reports Enabled` is a standard resolved column (`reportsEnabledIndex`/`headerIndex` unused on this path — they delete at cutover). Tally from the Referrals tab is unchanged. First-ever test coverage. | *Original entry:* Reads the **Referral Partners tab** and its extra `Reports Enabled` column. Becomes: filter the one table on `Category = 'Referral Partner'`, read `Reports Enabled` as a normal column, keep the existing skips (`Cold`/`Archive` status, `FALSE` opt-out, zero-referral partners). | 🟠 Medium. Currently untested, and a filter bug silently emails the wrong people — or nobody. |
